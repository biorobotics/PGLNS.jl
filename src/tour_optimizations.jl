...
function moveopt!(tour::Array{Int64, 1}, dist, sets::Vector{Vector{Int64}}, 
				  member, setdist::Distsv, set_locks::Vector{ReentrantLock}, lock_times::Vector{Float64}, thread_idx::Int64)
    ...
    @inbounds while improvement_found && number_of_moves < 10
        ...
        for i = start_position:length(tour)
            ...
            v, pos, cost = insert_cost_lb(tour, dist, sets[set_ind], set_ind, setdist, 
										  select_vertex, i, delete_cost, set_locks[set_ind], lock_times, thread_idx)
            insert!(tour, pos, v)
...
function moveopt_rand!(tour::Array{Int64, 1}, dist, sets::Vector{Vector{Int64}}, 
				  member, iters::Int, setdist::Distsv, set_locks::Vector{ReentrantLock}, lock_times::Vector{Float64}, thread_idx::Int64)
...
@inline function insert_cost_lb(tour::Array{Int64,1}, dist, set::Array{Int64, 1}, setind::Int, 
						   setdist::Distsv, bestv::Int, bestpos::Int, best_cost::Int, set_lock::ReentrantLock, lock_times::Vector{Float64}, thread_idx::Int64)

    set_copy = Vector{Int64}()
    bt = time()
    lock(set_lock)
    try
      at = time()
...
function opt_cycle!(current::Tour, dist, sets::Vector{Vector{Int64}}, 
					member, param::Dict{Symbol, Any}, setdist::Distsv, use, set_locks::Vector{ReentrantLock}, lock_times::Vector{Float64}, thread_idx::Int64)
	current.cost = tour_cost(current.tour, dist)
	prev_cost = current.cost
	for i=1:5
		if i % 2 == 1
			current.tour = reopt_tour(current.tour, dist, sets, member, param, set_locks, lock_times, thread_idx)
		elseif param[:mode] == "fast" || use == "partial"
			moveopt_rand!(current.tour, dist, sets, member, param[:max_removals], setdist, set_locks, lock_times, thread_idx)
		else
			moveopt!(current.tour, dist, sets, member, setdist, set_locks, lock_times, thread_idx)
		end
		current.cost = tour_cost(current.tour, dist)
...
function reopt_tour(tour::Array{Int64,1}, dist, sets::Vector{Vector{Int64}}, 
					member, param::Dict{Symbol, Any}, set_locks::Vector{ReentrantLock}, lock_times::Vector{Float64}, thread_idx::Int64)
  ...
  set_copy = Vector{Int64}()
  bt = time()
  lock(set_locks[member[tour[1]]])
  try
    at = time()
    lock_times[thread_idx] += at - bt
    set_copy = copy(sets[member[tour[1]]])
  finally
    unlock(set_locks[member[tour[1]]])
  end
  @inbounds for start_vertex in set_copy
    relax_in!(cost_to_come, dist, prev, Int64[start_vertex], sets[member[tour[2]]], ReentrantLock(), set_locks[member[tour[2]]], lock_times, thread_idx)
    for i = 3:length(tour)  # cost to get to ith set on path through (i-1)th set
        relax_in!(cost_to_come, dist, prev, sets[member[tour[i-1]]], sets[member[tour[i]]], set_locks[member[tour[i-1]]], set_locks[member[tour[i]]], lock_times, thread_idx)
    end
    # find the cost back to the start vertex.
    tour_cost, start_prev = relax(cost_to_come, dist, sets[member[tour[end]]], start_vertex, set_locks[member[tour[end]]], lock_times, thread_idx)
...
"""
@inline function relax(cost::Array{Int64, 1}, dist, set1::Array{Int64, 1}, v2::Int64, set_lock::ReentrantLock, lock_times::Vector{Float64}, thread_idx::Int64)
  set1_copy = Vector{Int64}()
  bt = time()
  lock(set_lock)
  try
    at = time()
	...
	min_prev = v1
  @inbounds for i = 2:length(set1_copy)
  v1 = set1_copy[i]
...
@inline function relax_in!(cost::Array{Int64, 1}, dist, prev::Array{Int64, 1}, 
				 set1::Array{Int64, 1}, set2::Array{Int64, 1}, set1_lock::ReentrantLock, set2_lock::ReentrantLock, lock_times::Vector{Float64}, thread_idx::Int64)
  set1_copy = Vector{Int64}()
  bt = time()
  lock(set1_lock)
  try
  ...
  try
	...
	@inbounds for v2 in set2_copy
		v1 = set1_copy[1]
		cost[v2] = cost[v1] + dist[v1, v2]
		prev[v2] = v1
    for i = 2:length(set1_copy)
			v1 = set1_copy[i]
			newcost = cost[v1] + dist[v1, v2]
      if cost[v2] > newcost
