...
function solver(problem_instance::String, given_initial_tours::AbstractArray{Int64,1}, start_time_for_tour_history::UInt64, inf_val::Int64, num_vertices::Int64, num_sets::Int64, sets::Vector{Vector{Int64}}, dist::AbstractArray{Int64,2}, membership::Vector{Int64}, instance_read_time::Float64, cost_mat_read_time::Float64, max_threads::Int64, powers::Dict{String,Any}=Dict{String,Any}(), update_powers::Bool=true, pin_cores::Vector{Int64}=Vector{Int64}(); args...)
  ...
  if length(given_initial_tours) != 0
	...
	count = Dict(:latest_improvement => Atomic{Int64}(1),
	  			 :first_improvement => Atomic{Bool}(false),
	 		     :warm_trial => Atomic{Int64}(0),
	  		     :cold_trial => 1,
				 :total_iter => Atomic{Int64}(0),
  ...
  sets_copies = [deepcopy(sets) for threads_idx=1:nthreads]

  tour_history = Array{Tuple{Float64, Array{Int64,1}, Int64},1}()
  num_trials_feasible = Atomic{Int64}(0)
  num_trials = Atomic{Int64}(0)
  ...
  lock_times = zeros(nthreads)
    ...
    while (@atomic count[:warm_trial].x) <= param[:warm_trials]
      iter_count = Atomic{Int64}(1)
      current = tour_copy(best)
      temperature = Atomic{Float64}(1.442 * param[:accept_percentage] * best.cost)
      ...
      if (@atomic count[:warm_trial].x) > 0	  # if warm restart, then use lower temperature
        @atomic temperature.x *= cooling_rate^(param[:num_iterations]/2)
        phase = :late
      end
      this_phase = phase
      thread_broke = false
      ...
      # for thread_idx=1:1 # Need to use this to match GLNS
        ...
        while true
          do_break = false
          if thread_broke || 
             ((@atomic count[:latest_improvement].x) > ((@atomic count[:first_improvement].x) ?
          ...
          try
            if (@atomic iter_count.x) > param[:num_iterations]/2 && phase == :early
          ...
          trial = remove_insert(current, dist, membership, setdist, sets_copies[thread_idx], powers, param, this_phase, powers_lock, current_lock, update_powers, lock_times, thread_idx)
          ...
          if param[:mode] == "slow"
            ...
            lock_times[thread_idx] += at - bt
            try
              accept = accepttrial_noparam(trial.cost, current.cost, param[:prob_accept]) || accepttrial(trial.cost, current.cost, (@atomic temperature.x))
            finally
              unlock(current_lock)
            end

            if accept
              opt_cycle!(trial, dist, sets_copies[thread_idx], membership, param, setdist, "full")
              bt = time()
              lock(current_lock)
              at = time()
              lock_times[thread_idx] += at - bt
            ...
            try
              if accepttrial_noparam(trial.cost, current.cost, param[:prob_accept]) ||
                 accepttrial(trial.cost, current.cost, (@atomic temperature.x))
                accept = true
                current = tour_copy(trial)
              else
                trial = tour_copy(current) # I don't remember if there was a point to doing this
          ...
          if updated_best
            @atomic count[:latest_improvement].x = 1
            @atomic count[:first_improvement].x = true
            if count[:cold_trial] > 1 && (@atomic count[:warm_trial].x) > 1
              @atomic count[:warm_trial].x = 1
            end
          else
            @atomic count[:latest_improvement].x += 1
          end
          ...
          @atomic temperature.x *= cooling_rate  # cool the temperature

          @atomic iter_count.x += 1
          @atomic count[:total_iter].x += 1

          if time() - init_time > param[:max_time]
            break
          end
        end
        @atomic num_trials.x += this_num_trials
        @atomic num_trials_feasible.x += this_num_trials_feasible
      end
      print_warm_trial(count, param, best, iter_count.x)
      @atomic count[:warm_trial].x += 1
      @atomic count[:latest_improvement].x = 1
      @atomic count[:first_improvement].x = false
  ...
  print_summary(lowest, timer, membership, param, tour_history, cost_mat_read_time, instance_read_time, (@atomic num_trials_feasible.x), (@atomic num_trials.x), param[:timeout], lock_times)
