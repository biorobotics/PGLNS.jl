...
function remove_insert(current::Tour, dist, member,
						...
						powers, param::Dict{Symbol,Any}, phase::Symbol, powers_lock::ReentrantLock, current_lock::ReentrantLock, update_powers::Bool, lock_times::Vector{Float64}, thread_idx::Int64)
  ...
  # noise = powers["noise"][noise_idx]
	...
	if insertion.name == "cheapest"
		cheapest_insertion!(trial.tour, sets_to_insert, dist, setdist)
	else
