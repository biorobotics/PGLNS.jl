...
function VDNodeAstar(tour_idx::Int64, parent::Vector{VDNodeAstar}, visited_removed_sets::Vector{Bool}, final_node_idx::Int64, g_val::Int64, h_val::Int64, id::Int64)
  ...
  return VDNodeAstar(tour_idx, parent, visited_removed_sets, final_node_idx, (tour_idx, visited_removed_sets, final_node_idx), g_val, h_val, g_val + h_val, id)
end
...
function astar_insertion!(sets_to_insert::Vector{Int64}, dist::AbstractArray{Int64, 2}, sets::Vector{Vector{Int64}}, membership::Vector{Int64}, inf_val::Int64, stop_time::Float64, vd_info::VDInfo, partial_tour::Vector{Int64}, ub::Int64)
  ...
  root_node = VDNodeAstar(1, Vector{VDNodeAstar}(), zeros(Bool, length(sets_to_insert)), 1, 0, 0, vd_node_id)
  vd_node_id += 1
  ...
  goal_node = VDNodeAstar(0, Vector{VDNodeAstar}(), zeros(Bool, 1), 1, typemax(Int64), 0, 0)
  while length(open_list) != 0 && goal_node.f_val > peek(open_list).first.f_val
    ...
    if haskey(seen_nodes, pop.key) && pop.id != seen_nodes[pop.key].id
      # at = time_ns()
    ...
    for removed_set_idx in unvisited_removed_sets
      ...
      for node_idx in this_set
        ...
        # bt = time_ns()
        neighbor_node = VDNodeAstar(pop.tour_idx + 1, [pop], copy(pop.visited_removed_sets), node_idx, pop.g_val + dist[pop.final_node_idx, node_idx], w*h_val, vd_node_id)
