...
function main()
  ...
  for i=1:2
    ARGS = [expanduser("~/GLKH-1.1/GTSPLIB/"*instance_folder*"/custom"*string(i)*".gtsp"), "-output=custom.tour", "-socket_port=65432", "-new_socket_each_instance=0", "-verbose=3", "-mode=fast", "-num_iterations=60"]
    ...
    @time GLNS.main(ARGS, 10., inf_val, given_initial_tours, dist, 10)
    @time GLNS.main(ARGS, 10., inf_val, PyArray{Int64, 1, true, true, Int64}(given_initial_tours), PyArray{Int64, 2, true, true, Int64}(dist), 10)
  end
end
